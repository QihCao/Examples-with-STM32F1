...
int main(void)
{
  ...
  while (1)
  {
	/*	uint16_t speed = 800; // Ði?u ch?nh giá tr? này d? di?u khi?n t?c d?
    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, speed);
		__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, speed);
    ...
    HAL_GPIO_WritePin(IN4_PORT, IN4_PIN, GPIO_PIN_SET);*/
      ...
      Motor_Forward(800);
      HAL_Delay(1000); // Ð?i 2 giây

        // Ði?u khi?n d?ng co lùi 
       Motor_Reverse(800);
       HAL_Delay(1000); // Ð?i 2 giây

        // Ði?u khi?n d?ng co quay trái 
     /*   Motor_Left(800);
        HAL_Delay(500); // Ð?i 2 giây
